...
$LogFile = Join-Path $LogDir "r2_scheduler_new_taipei.log"
$stamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

$Python = Join-Path $ProjectDir ".venv\Scripts\python.exe"
if (-not (Test-Path $Python)) { $Python = "python" }

# 新北生产路径：research-driven 研判生成（config = election_assessment.new_taipei.yaml）
$argsList = @("-m", "app.assessment.research_driven.scheduled", "--config", "config/election_assessment.new_taipei.yaml", "--trigger-type", $TriggerType)
